...
import com.cburch.logisim.gui.menu.HelpFrame;
...
public class AttrTable extends JPanel implements LocaleListener, ActionListener {
	...
	public AttrTable(Window parent) {
		...
		titleEnabled = true;
		help = new JButton(Icons.getIcon("help.png"));
		help.setPreferredSize(new Dimension(26, 26));
		help.addActionListener(this);
		help.setToolTipText("Help for component, Library Reference.");
	...
	{
		String page = tableModel.attrModel.getHelpPage();
		showHelp(page);
	}
	...
	private void updateTitle() {
		if (titleEnabled) {
			...
			String helpText = tableModel.attrModel.getHelpPage();
			if (text == null) {
				title.setVisible(false);
				help.setVisible(false);
			} else {
				title.setText(text);
				help.setToolTipText(String.format("Help for %s, Library Reference.", text));
	...
	private void loadBroker()
	{
		try
		{
			if (!HelpFrame.instance().loadBroker())
			{
				JOptionPane.showMessageDialog(null, Strings.get("helpNotFoundError"));
				return;
			}
		}
		catch (Exception e)
	...
	private void showHelp(String target)
	{
		...
		try
		{
			HelpFrame.instance().setCurrentID(target);
		...
		{
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, Strings.get("helpDisplayError"));
		}
	}
}
