...
public class Counter extends InstanceFactory {
	...
	public void propagate(InstanceState state) {
		...
		if (data == null) {
			...
			state.setData(data);
			data.setValue(Value.createKnown(dataWidth, 0));
		}
		...
		if (state.getPort(CLR) == Value.TRUE) {
			...
			if (!triggered) {
				...
				int goal = ld ? 0 : max;
				if (oldVal.isFullyDefined() && oldVal.toIntValue() == goal) {
					...
					if (onGoal == ON_GOAL_WRAP) {
						...
						newVal = Value.createKnown(dataWidth, oldVal.toIntValue() + (ld ? -1 : 1));
					} else {
						System.err.println("Invalid goal attribute " + onGoal); //OK
						newVal = Value.createKnown(dataWidth, ld ? max : 0);
					}
				} else {
					if (oldVal.isFullyDefined())
						newVal = Value.createKnown(dataWidth, oldVal.toIntValue() + (ld ? -1 : 1));
					else {
						if (ld)
						{
			...
			if (newVal.isFullyDefined())
				carry = newVal.toIntValue() == (ld && ct ? 0 : max);
			else 
				carry = false;
			newValue = newVal;
	...
	public void paintInstance(InstancePainter painter) {
		...
		if (painter.getShowState()) {
			...
			String str = (state == null) ? StringUtil.toHexString(width, 0) : state.value.toHexString();//
			if (str.length() <= 4) {
				a = str;
			} else {
	...
	public String getHelpPage() { return "mem_counter"; }
}