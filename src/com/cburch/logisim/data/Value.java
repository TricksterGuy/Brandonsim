...
public class Value
{
	...
	private static final Color UNKNOWN_MULTI_COLOR = new Color(40, 40, 128);
	...
	public Value set(int which, Value val)
	{
		...
		{
			int mask = ~(1 << which);
			return Value.create(this.width, (this.error & mask) | (val.error << which), (this.unknown & mask)
					| (val.unknown << which), (this.value & mask) | (val.value << which));
		}
	}
	...
	public Value combine(Value other)
	{
		...
		else
		{
			...
			return Value.create(Math.max(this.width, other.width), this.error | other.error | disagree, this.unknown
					& other.unknown, (this.value & ~this.unknown) | (other.value & ~other.unknown));
		}
	}
	...
	public Value and(Value other)
	{
		...
		{
			return Value.create(Math.max(this.width, other.width), this.error | other.error,
					(this.unknown & (other.unknown | other.value)) | (other.unknown & (this.unknown | this.value)),
					this.value & other.value);
		}
	}
	...
	public Value or(Value other)
	{
		...
		{
			return Value.create(Math.max(this.width, other.width), this.error | other.error,
					(this.unknown | other.unknown) & ~(this.value | other.value), this.value | other.value);
		}
	}
	...
	public Value xor(Value other)
	{
		...
		{
			if (this == ERROR || other == ERROR) return ERROR;
			if (this == NIL || other == NIL) return UNKNOWN;
		...
		{
			return Value.create(Math.max(this.width, other.width), this.error | other.error, this.unknown
					| other.unknown, this.value ^ other.value);
		}
	}
	...
	public Value add(int i)
	{
		if (width == 1)
		{
			...
			return Value.create(width, error, unknown, value + i);
		}
	}
}
